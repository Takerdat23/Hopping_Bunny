...
gltfLoader.load(
  // 'https://github.khronos.org/glTF-Sample-Viewer-Release/assets/models/Models/AnimatedMorphCube/glTF/AnimatedMorphCube.gltf', // Ensure this path is correct
  ...
  function (gltf) {
    ...
    cottageModel.scale.set(20, 20, 20);
...
function createFloor() {
  ...
  // Duplicate for the second floor segment
  floorShadow2 = floorShadow1.clone();
  floorGrass2 = floorGrass1.clone();

  floor2 = new THREE.Group();
  floor2.position.y = 0;
  floor2.position.x = floorRadius * 4;

  floor2.add(floorShadow2);
  floor2.add(floorGrass2);
...
class Hero {
  ...
  jump() {
    ...
    this.status = "jumping";
    var _this = this;
    var totalSpeed = 5 / speed;
    var jumpHeight = 35;

    TweenMax.to(this.mesh.position, totalSpeed / 2, { y: jumpHeight, ease: Power3.easeInOut });
...
function createCottages() {
  ...
  var nCottages = 2; // Number of cottages
  ...
  for (var i = 0; i < nCottages; i++) {
    ...
    cottage.rotation.y = Math.random()

    cottage.position.z = -Math.random() * 100 - 50
    floor1.add(cottage);
    ...
    cottage2.rotation.y = Math.random()

    cottage2.position.z = -Math.random() * 100 - 50
...
function updateFloorRotation() {
  floor1.position.x -= delta * speed * 10;
  floor2.position.x -= delta * speed * 10;

  // Check if floor segments have moved out of view and reset their positions
  if (floor1.position.x <= -floorRadius * 4) {
    floor1.position.x = floor2.position.x + floorRadius * 4;
    resetTrees(floor1); // Reset trees when the floor segment resets
  }
  if (floor2.position.x <= -floorRadius * 4) {
    floor2.position.x = floor1.position.x + floorRadius * 4;
    resetTrees(floor2); // Reset trees when the floor segment resets
  }
}

// Function to reset tree positions within a floor segment
function resetTrees(floor) {
  floor.children.forEach(child => {
    if (child instanceof THREE.Group && child !== floorShadow1 && child !== floorGrass1 && child !== floorShadow2 && child !== floorGrass2) {
      child.position.z = -floorRadius * 1.5 + Math.random() * floorRadius * 3;
